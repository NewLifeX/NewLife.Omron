...
/// <summary>CpuUnitData和CpuUnitStatus模型测试</summary>
public class CpuModelTests
{
    [Fact(DisplayName = "CpuUnitData解析")]
    public void ParseCpuUnitData()
    {
        // 构建模拟数据：型号(20) + 版本(4) + 系统版本(4) + 容量(2)
    ...
    [Fact(DisplayName = "CpuUnitData数据不足应抛出异常")]
    ...
    [Fact(DisplayName = "CpuUnitStatus解析运行模式")]
    ...
    [Fact(DisplayName = "CpuUnitStatus解析错误状态")]
    public void ParseCpuUnitStatusError()
    {
        var data = new Byte[8];
        data[0] = 0x00; // Program模式
        data[1] = 0x01; // 致命错误
        data[2] = 0x00;
        data[3] = 0x10; // 错误数据
        data[4] = 0x01; // 非致命错误
    ...
    [Fact(DisplayName = "CpuUnitStatus数据不足应抛出异常")]
